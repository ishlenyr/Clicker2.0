class audioController {
  constructor(game) {
    this.game = game;
    this.backgroundMusic = new Audio("src/audio/MainTheme.mp3");
  ...
  get musicVolume() {
    return this.game.settings.muteSounds === true
      ? 0
      : this.game.settings.musicVolume / 100;
  }

  get effectVolume() {
    return this.game.settings.muteSounds === true
      ? 0
      : this.game.settings.effectVolume / 100;
  }

  playSoundIndependently(soundName) {
    const sound = new Audio(`src/audio/${soundName}`);
